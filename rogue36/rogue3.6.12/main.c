...
/* {{ */
    ...
    whoami[0] = '\0';
#ifdef _WIN32
    sprintf(cfgfile, "%s\\rogue36.cfg", getenv("APPDATA"));
#else
    sprintf(cfgfile, "%s/rogue36.cfg", getenv("HOME"));
#endif
	...
	rs_read_int(fptr, &slow_invent);
	rs_read_int(fptr, &askme);
	rs_read_string(fptr, whoami, 80);
	rs_read_string(fptr, fruit, 80);
	fclose(fptr);
    }
/* }} */
