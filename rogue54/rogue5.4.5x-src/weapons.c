...
static struct init_weaps {
    ...
    { "1x2",	"2x4",	NO_WEAPON,	ISMANY|ISMISL,	},	/* Shuriken (Crossbow bolt) */
    { "2x3",	"1x6",	NO_WEAPON,	ISMISL,		},	/* Spear */
    { "1x1",	"1x1",	NO_WEAPON,	0,		},	/* (Crossbow) */
};
static char bolt_iw_hrl[] = "2x5";
...
init_weapon2(void)
{
	...
	if (weap_info2[i].oi_name[0] != '\0')
	    weap_info[i].oi_name = weap_info2[i].oi_name;
	if (weap_info2[i].oi_prob != -1)
	    weap_info[i].oi_prob = weap_info2[i].oi_prob;
	if (weap_info2[i].oi_worth != -1)
	    weap_info[i].oi_worth = weap_info2[i].oi_worth;
    }
    init_dam[BOLT].iw_hrl = bolt_iw_hrl;
    init_dam[BOLT].iw_launch = CROSSBOW;
}
