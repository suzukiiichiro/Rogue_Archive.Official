...
#ifdef MAC
// very mac dependent routine
// all objects---> inventToDrawer(&rogue.pack, ALL_OBJECTS);
// descs[24][80]
inventToDrawer(void)
{
#define	Protected(obj)	((obj->what_is & ARMOR) && obj->is_protected)
	...
	short row, col;
	char *p;
	object *pack = &rogue.pack;
	extern ControlRef itemListRef;
	unsigned int mask = ALL_OBJECTS;
	i = 0;
	maxlen = 80;
	...
	for (j=0; j<i; j++){
		listData[j]=descs[j];
		//AddDataBrowserItems( itemListRef, kDataBrowserNoItem, i, descs[j], kDataBrowserItemNoProperty);
		//SetDataBrowserPropertyFlags( itemListRef, 'data' , kDataBrowserPropertyTextPart);
	}
	updateList();
