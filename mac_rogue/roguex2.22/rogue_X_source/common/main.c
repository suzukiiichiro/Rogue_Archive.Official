...
char **av;
{
	...
	if (ac < 2) {
#ifdef MAC
		maccur_alert_msg("%s: message_file [options...]\n", av[0]);
		macResetMessage();
		return read_default_mesg();
#else
...
#ifdef MAC
		maccur_alert_msg("Cannot open message file '%s'\n", av[1]);
		macResetMessage();
		return read_default_mesg();
#else
...
FMTERR:				
#ifdef MAC
					maccur_alert_msg("Illegal format '%s'\ndefault message used\n",
					av[1]);
					macResetMessage();
					fclose(mesg_file);
					return read_default_mesg();
