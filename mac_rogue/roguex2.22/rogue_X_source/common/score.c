...
#ifdef JAPAN
char *score_file = "jrogue.scores";
#else
char *score_file = "rogue.scores";
#endif
extern	long mySignature;
#ifdef __MWERKS__
extern	long _fcreator;	//	Creator/type val of CW MSC-LIB
extern	long _ftype;
#endif
#endif
...
/*	static char xpos[] = {35,34,33,32,31,30,30,30,30,30,30,30,29,21};*/
	static char *str[] = {
		         ...
		         "|                  |",
		        "*|     *  *  *      | *",
		"________)/ÄÄ_//(Ä/(/Ä)/Ä//Ä/|_)_______"
#else
		              "----------",
		             "/          \\",
		            "/            \\",
...
boolean from_intrpt;
{
	...
	check_message();
#ifdef JAPAN
	message("ÉQÅ[ÉÄÇèIóπÇµÇƒÇÊÇ¢ÇÃÇ≈Ç∑Ç©ÅH", 1);
#else
...
#ifdef MAC
	{
		CFArrayRef scoreRef,nameRef;
		CFDataRef scoreData[10],nameData[10];
		CFIndex aCount;
		CFRange allRange;
		scoreRef = CFPreferencesCopyAppValue( CFSTR("Scores"),kCFPreferencesCurrentApplication);
		if (scoreRef) {
			nameRef = CFPreferencesCopyAppValue( CFSTR("Names"),kCFPreferencesCurrentApplication);
			//aCount = CFArrayGetCount(scoreRef);
			allRange.location = 0;
			allRange.length = CFArrayGetCount(scoreRef);
			CFArrayGetValues(scoreRef,allRange, scoreData);
			CFArrayGetValues(nameRef,allRange, nameData);
			(void) xxx(1);
			for (i = 0; i < allRange.length; i++) {
				CFDataGetBytes(scoreData[i],CFRangeMake(0,80),scores[i]);
				CFDataGetBytes(nameData[i],CFRangeMake(0,30),n_names[i]);
				xxxx(scores[i], 80);
				xxxx(n_names[i], 30);
				}
		}else {
...
//#ifdef __MRC__
//---MPW stdCì∆é©ÇÃä÷êî
	//fsetfileinfo (file, mySignature, 'rgSc');
//#endif
#endif
	ne = i;
...
#endif
#ifdef MAC // Use Preferenes of MacOS X
				{
					CFDataRef scoreRef[ne], n_nameRef[ne];
					CFArrayRef scoreArray, nameArray;
					(void) xxx(1);
					for (i = 0; i < ne; i++) {
						xxxx(scores[i], 80);
						scoreRef[i]= CFDataCreate(NULL,(void*)(scores[i]), 80);
						xxxx(n_names[i], 30);
						n_nameRef[i]= CFDataCreate(NULL,(void*)( n_names[i]), 30);
					}
					// Score data copyed to dataBuffer
					scoreArray = CFArrayCreate( kCFAllocatorDefault, 
							(const void **)scoreRef, 
							ne,
							&kCFTypeArrayCallBacks ); 
					nameArray = CFArrayCreate( kCFAllocatorDefault, 
							(const void **)n_nameRef, 
							ne,
							&kCFTypeArrayCallBacks );
					CFPreferencesSetValue(CFSTR("Scores"), scoreArray,
							kCFPreferencesCurrentApplication,
							kCFPreferencesAnyUser,
							kCFPreferencesAnyHost
							);
					CFPreferencesSetValue(CFSTR("Names"), nameArray,
							kCFPreferencesCurrentApplication,
							kCFPreferencesAnyUser,
							kCFPreferencesAnyHost
							);
...
short other;
{
	...
	{
		CFArrayRef scoreRef,nameRef;
		CFData scoreData[10],nameData[10];
		CFIndex aCount;
		CFRange allRange;
		scoreRef = CFPreferencesCopyAppValue( CFSTR("Scores"),kCFPreferencesCurrentApplication);
		nameRef = CFPreferencesCopyAppValue( CFSTR("Names"),kCFPreferencesCurrentApplication);
