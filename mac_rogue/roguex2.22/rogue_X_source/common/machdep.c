...
/*
	...
	clen = strlen(fname);
	if (clen > 255){
		DebugStr("\pPathname is too long");
		clen = 255;
	}
	pname[0] = (unsigned char) clen;
	BlockMoveData((Ptr)fname, (Ptr)&(pname[1]),clen);
	err = FSMakeFSSpec(0,0,pname,&fp);
	...
	err = PBGetCatInfoSync((CInfoPBPtr)&fi);
	seconds = fi.ioFlMdDat;
	t = localtime(&seconds);
	rt_buf->year = t->tm_year;
	rt_buf->month = t->tm_mon + 1;
	rt_buf->day = t->tm_mday;
...
macgetlogin()
{
	...
	cfName = CSCopyUserName(FALSE);

	ok = CFStringGetCString(cfName,buf,BUFSIZ-1, encoding);
	CFRelease(cfName);
	if (ok) return buf;
	else {
		// if can't convert Unicode to Cstring, Try again with short login name.
		cfName = CSCopyUserName(TRUE);
		ok = CFStringGetCString(cfName,buf,BUFSIZ-1, encoding);
		CFRelease(cfName);
		if (ok) return buf;
	}
...
char *name;
{
#ifdef MAC	/* Use file as env */
	extern char* macGetEnv(char* envname);
	return macGetEnv(name);
#else
	char *getenv();
...
int status;
{
	...
	char *opt = NULL;
	extern char *buildCurrentOptStr(void);
	extern void saveFontSetting(char* fontname, int fontsize);
	extern char *_maccur_font_name;
	extern int _maccur_font_size;
	extern void saveWindowPosition(void);
	extern Boolean gUseNavigation;
	CFStringRef optRef;
	Handle h = NULL;
	//printf("-----%s\n", _maccur_font_name);
	opt = buildCurrentOptStr();
#ifdef JAPAN
	optRef = CFStringCreateWithCString(NULL, opt, kCFStringEncodingMacJapanese);
#else
	optRef = CFStringCreateWithCString(NULL, opt, kCFStringEncodingMacRoman);
#endif
	CFPreferencesSetAppValue(CFSTR("ROGUEOPTS"), optRef, kCFPreferencesCurrentApplication);
