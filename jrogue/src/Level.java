...
if(0!=(mask&TUNNEL))return '$';//FOOP
	...
	Toy gr_wand(){
		return new Toy(this, Id.WAND + self.rand.get(Id.id_wands.length-1));
	}
	...
	void draw_magic_map(Man man){
		...
		for(int j= 0; j < ncol; j++){
			...
			if(0 !=(s & (HORWALL|VERTWALL|DOOR|TUNNEL|TRAP|STAIRS))){
				if(0==(s & MONSTER)){
					man.seen[i][j]= (char)((man.seen[i][j]&15)
					                     |man.wallcode(get_char(i,j)));
					man.view.mark(i, j);
				}
			}
		}
	...
	void bounce(Toy obj, int dir, int row, int col, int r){
		...
		if(0 != (map[row][col] & MONSTER)){
			...
			for(Enumeration e= self.view_list.elements(); e.hasMoreElements();){
				Man man= (Man)((View)e.nextElement()).man;
	...
	void wake_room(Man man, boolean entering, int row, int col){
		...
		while(e.hasMoreElements()){
			...
			&& sees(row, col, monster.row, monster.col))
