public class Room implements Header{
	...
	public String toString(){
		...
		if(0 != (is_room & R_NOTHING))	s= s.concat(" Nothing");
		if(0 != (is_room & R_ROOM))		s= s.concat(" Room");
		if(0 != (is_room & R_MAZE))		s= s.concat(" Maze");
		if(0 != (is_room & R_DEADEND))	s= s.concat(" Deadend");
		if(0 != (is_room & R_CROSS))	s= s.concat(" Cross");
		if(0 != (is_room & R_VISIT))	s= s.concat(" Visit");
		return s + ']';
	}
	Room(){	// A named place-holder
 	...
 	Room(int rn, int left_col, int top_row, int width, int height, Level level, boolean isnothing){
		...
		if(!isnothing){
			...
			for(j= left_col+1; j <= right_col-1; j++)
	...
	void make_maze(int r, int c){
		...
		for(i= 0; i < 4; i++){
			...
			case Id.UPWARD:
				if(r-1 >= top_row
				&& level.map[r-1][c]!= (TUNNEL | HOLDER)
				&& level.map[r-1][c-1] != (TUNNEL | HOLDER)
				&& level.map[r-1][c+1] != (TUNNEL | HOLDER)
	...
	Rowcol put_door(int dir){
		...
		switch(dir){
		case Id.UPWARD:	row= top_row; break;
		case Id.DOWN:		row= bottom_row; break;
		case Id.RIGHT:		col= right_col; break;
		case Id.LEFT:		col= left_col; break;
		}
		if(row>=0){
			do
	...
	boolean settunnel(int r, int c){
		if(0!=(level.map[r][c]&(HORWALL|VERTWALL|FLOOR|HOLDER|STAIRS))){
			if(0!=(level.map[r][c]&(HORWALL|VERTWALL))){
				...
				Door tovoid= new Door(level, r, c, 0, 0);
				level.map[r][c]= DOOR;
			}
	...
	void visit_rooms(){
		...
		r.is_room |= R_VISIT;
		for(int i= 0; i<4; i++)if(doors[i]!=null){
	...
	void fill_it(boolean do_rec_de){
		...
		for(i= 0; i < 4; i++){
			...
			Door tovoid= new Door(level, pd.row, pd.col, ps.row, ps.col);
			rooms_found++;
