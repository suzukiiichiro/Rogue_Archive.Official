...
class Scroll extends Toy {
		...
		switch(kind){
			...
			case Id.IDENTIFY:
				if(owner instanceof Man){
					...
					identified= true;
					Id.id_scrolls[kind&255].id_status= Id.IDENTIFIED;
	...
	boolean create_monster(Persona man){
		...
		for(int i= 0; i<9; i++){
			...
			if((r==man.row && c==man.col)
			|| r<MIN_ROW || r >level.nrow-2
			|| c<0 || c>level.ncol-1)
				continue;
			if(0==(level.map[r][c] & MONSTER)
			&& 0!=(level.map[r][c] & (FLOOR|TUNNEL|STAIRS|DOOR))){
	...
	void aggravate(Persona man){
		Enumeration e= level.level_monsters.elements();
