...
class Man extends Persona{
	...
	int play_move(int ch, int count){
		...
		case 'B':	case 'Y':	case 'U':	case 'N':
			...
			break;
/*
		case '\010':	case '\012':	case '\013':	case '\014':
		case '\031':	case '\025':	case '\016':	case '\002':
			do {
				r= row;
				c= col;
			...
			if(!self.interrupted && passgo && m==MOVE_FAILED && 0!=(level.map[row][ col]&TUNNEL))
...
*/
		...
		default:
			...
			case 'E'-'@':
				level.show_traps(this); break;
			case 'O'-'@':
				level.show_toys(this); break;
			case 'C'-'@':
				c_toy_for_wizard(); break;
			case 'X'-'@':
				monster_for_wizard(); break;
			case 'Q'-'@':
				level.show_monsters(this); break;
			case 'T'-'@':
				tele(); break;
			case 'W'-'@':
	...
	void play_level(){
 	...
 	char showrc(int r, int c){	// What man sees at this position
		...
		// Error if blind and toy in tunnel or on door--see blank instead of tunnel floor/door
		return ch;
	}
	void showmap(){
		Rowcol pt;
...
//System.out.println("In pack_letter " + ch);
	...
	void player_init(){
		pack= new ItemVector(MAX_PACK_COUNT);
