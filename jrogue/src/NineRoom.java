...
class NineRoom extends Level {
	...
	NineRoom(int nrow, int ncol, Rogue self){
		...
		while(!bfinished)try{
			...
			if(big_room){
				...
				room.addElement((Object)(new Room(BIG_ROOM, left_col, top_row, width, height, this, false)));
			}else {
				room= new Vector(MAXROOMS);
				...
				for(i= 0; i < MAXROOMS; i++){
					...
					room.addElement((Object)(new Room(i, left_col, top_row,
						width, height, this, isnothing)));
				}
	//int nx= 0; for(j= 0; j<MAXROOMS; j++)if(((Room)room.elementAt(j)).is_room==Room.R_NOTHING)++nx;
	//System.out.println("Missing " + nx);
				add_mazes();
				int perm[]= self.rand.permute(MAXROOMS);
	
				for(j= 0; j < MAXROOMS; j++){
					r= (Room)room.elementAt(perm[j]);
					connect(r, 1);
					connect(r, 3);
					if(r.rn + 2<room.size()
					&& ((Room)room.elementAt(r.rn+1)).is_room==Room.R_NOTHING){
						if(connect(r, 2)){
							Room ra= (Room)room.elementAt(r.rn+1);
							ra.is_room= Room.R_CROSS;
						}
					}
					if(r.rn + 6<room.size()
					&& ((Room)room.elementAt(r.rn+3)).is_room==Room.R_NOTHING){
						if(connect(r, 6)){
							Room ra= (Room)room.elementAt(r.rn+3);
							ra.is_room= Room.R_CROSS;
						}
					}
					if(is_all_connected())
			...
			for(i= 0; i<room.size(); i++)
				((Room)room.elementAt(i)).put_gold(GOLD_PERCENT);		
	
			/*for(j= 0; j<level_doors.size(); j++){
				Door d= (Door)level_doors.elementAt(j);
				Room r1= room_at(d.row, d.col);
				Room r2= room_at(d.oth.row, d.oth.col);
				System.out.print(" " + get_char(d.row,d.col) + get_char(d.oth.row,d.oth.col));
	...
	private void add_mazes(){
		if(cur_level > 1){
			...
			int i= room.size();
			while(--i>=0){
				Room rj= (Room)room.elementAt(i);
				if(rj.is_room == rj.R_NOTHING){
					if(self.rand.percent(maze_percent)){
						rj.is_room= rj.R_MAZE;
						if(rj.left_col<2)
							rj.left_col= 2;
						int rv= self.rand.get(rj.top_row+1, rj.bottom_row-1);
						int cv= self.rand.get(rj.left_col+1, rj.right_col-1);
						rj.make_maze(rv,cv);
						rj.hide_boxed_passage(rj.top_row, rj.left_col,
							rj.bottom_row, rj.right_col,
							self.rand.get(2));
					}
				}
			}
	...
	void fill_out_level(){
		...
		Room.endroom= null;
		while(--i>=0){
			Room r= (Room)room.elementAt(perm[i]);
			if(r != null
	...
	Room gr_room(){
		...
		for(int i= 0; i<room.size(); i++){
			Room rm= (Room)room.elementAt(perm[i]);
		...
		for(i= 0; i < n; i++){
			if(i==0 && party_room!=null){
				do {
					...
					tries++;
				}while(0!=(map[row][col] & (TOY|STAIRS|TRAP|TUNNEL))
				    || (0==(map[row][col] & SOMETHING) && tries < 15));
			}
			if(tries==0 || tries >= 15){
			...
			Trap trap= new Trap(this, row, col, self.rand.get(Trap.TRAPS-1));
			map[row][col] |= HIDDEN;
			//System.out.println("Trap at " + row + " " + col + " (" + trap.m_flags + ")");
		}
	}
	boolean connect(Room rfr, int n){
		...
		if(rfr.rn+n>=room.size())
			return false;
		Room rto= (Room)room.elementAt(rfr.rn+n);
		if(0==(rfr.is_room & (Room.R_ROOM | Room.R_MAZE))
		|| 0==(rto.is_room & (Room.R_ROOM | Room.R_MAZE)))
			return false;
	...
	private Room nth_room(int n){
		return n>=0 && n<room.size()? (Room)room.elementAt(n) : null;
	}
	Room nabes(Room r)[]{
		Room ra[]= new Room[4];
